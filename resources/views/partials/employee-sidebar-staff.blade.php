...
<div id="sidebar"
    ...
    <div class="flex items-center justify-between p-4 border-b border-slate-700">
        <div class="flex items-center gap-2">
            ...
            <div>
                ...
                <span class="text-[10px] bg-green-500 text-white px-1.5 py-0.5
                             rounded font-semibold">STAFF</span>
            </div>
        </div>
    ...
    @php
        ...
        // CORRECT: guard('employee') returns Employee model
        // WRONG:   auth()->user() returns User model
        $employee = Auth::guard('employee')->user();
    @endphp

    {{-- <div class="p-4 border-b border-slate-700">
        <div class="flex items-center gap-3">
            <div class="h-10 w-10 rounded-full bg-green-600 flex items-center
    ...
    <nav class="p-3 space-y-1">
        ...
        <a href="{{ route('employee.staff.applications.index') }}"
           class="flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm
                  transition-all
                  {{ request()->routeIs('employee.staff.applications.*')
                     ? 'bg-green-600 text-white font-semibold'
                     : 'text-slate-300 hover:text-white hover:bg-slate-700' }}">
