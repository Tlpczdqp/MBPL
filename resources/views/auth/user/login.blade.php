{{--
    Extends the two-column auth layout.
    The left panel is the dark decorative side.
    The right panel (this file) contains the login form.
--}}
...
@section('content')
    ...
    {{-- Validation errors (shown if login fails) --}}
    @if($errors->any())
    ...
    {{-- Session messages (e.g., "verify your email first") --}}
    @if(session('error'))
        <div class="bg-red-50 border border-red-200 text-red-700 text-sm rounded-lg px-4 py-3 mb-5">
            {{ session('error') }}
        </div>
    ...
    <form method="POST" action="{{ route('user.login.submit') }}" class="space-y-4">
        ...
        {{-- Password --}}
        {{-- x-data and x-show let us toggle password visibility without page reload --}}
        <div x-data="{ showPass: false }">
            ...
            <div class="relative">
                ...
                {{-- :type binding changes between 'password' and 'text' based on showPass --}}
                <input id="password"
        ...
        {{-- Submit --}}
        <button type="submit"
                class="w-full py-2.5 px-4 bg-slate-900 hover:bg-slate-700 text-white text-sm
    ...
    <div class="relative my-6">
        ...
        <div class="relative flex justify-center text-xs text-slate-400 uppercase tracking-wider">
    ...
    <div class="grid grid-cols-2 gap-3">
        ...
        <a href="{{ route('auth.google') }}"
           class="flex items-center justify-center gap-2 px-4 py-2.5 rounded-lg border
    ...
    <p class="text-center text-xs text-slate-400 mt-3">
        ...
        <a href="{{ route('employee.login') }}"
           class="text-slate-600 hover:underline">Employee Login →</a>
    </p>
